...
async def download_instagram(url: str):
    ...
    try:
        ...
        # 🆕 المنطق المُعدَّل لتمييز الفيديو عن الصورة المصغرة والصور الأخرى
        if is_video:
            # إذا كان المنشور فيديو، ابحث عن ملف MP4 فقط
            for f in files:
                if f.endswith('.mp4'):
                    media_files.append(os.path.join(post_dir, f))
        else:
            # إذا كان المنشور صورة/ألبوم، ابحث عن الصور فقط (واستبعد الصور المصغرة التي قد لا تكون لها حاجة)
            # Instaloader يحفظ الصور المصغرة عادةً كـ shortcode.jpg
            for f in files:
                # استبعاد الملفات التي قد تكون صورًا مصغرة للمنشورات
                if f.endswith(('.jpg', '.png')) and not f.startswith('.'): # استبعاد أي ملفات مخفية
                     media_files.append(os.path.join(post_dir, f))

        if not media_files:
            return None, "❌ لم يتم العثور على ملفات وسائط صالحة في المنشور."
        
        # ترتيب الملفات المرسلة (مفيد للألبومات)
        media_files.sort()
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "tiktok.com" in url:
        ...
        if media_files:
            ...
            # 🆕 تنظيف جميع الملفات في مجلد المنشور، بما في ذلك الصورة المصغرة التي تم تجاهلها
